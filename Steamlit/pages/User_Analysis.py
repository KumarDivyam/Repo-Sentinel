...
def detect_outliers(df, parameter, contamination=0.05, custom_cmap='coolwarm'):
    ...
    scatter = ax.scatter(df.index, df_numeric[parameter], s=100, label=parameter, alpha=0.5, c=outliers, cmap=custom_cmap)
    ...
    outlier_names = [name if is_outlier else '' for name, is_outlier in zip(df['Contributor'], outliers)]
    for i, name in enumerate(outlier_names):
        if name:
            ax.annotate(name, (df.index[i], df_numeric[parameter][i]), color='red')
...
if uploaded_file is not None:
    ...
    for parameter in parameters:
        ...
        custom_cmap = custom_colormaps.get(parameter, 'coolwarm')  # Default to 'coolwarm'
    ...
    for parameter in outlier_info:
        ...
        for outlier_name in outlier_info[parameter]:
            ...
            outlier_value = df[df['Contributor'] == outlier_name][parameter].values[0]
            outlier_float = float(outlier_value)
            outlier_type.append('Negative' if abs(outlier_float) < threshold else 'Positive')

        # Create a list of dictionaries for each parameter
        parameter_outliers.extend(
            [{"Parameter": parameter, "Outlier": contributor, "Type": outlier_type[i]} for i, contributor in enumerate(outlier_info[parameter])]
        )
...
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ...
    most_common_outliers = [outlier for outlier, count in outlier_counts.items() if count == max_count]

    # Show the most occurring outliers
    st.subheader("Most Occurring Outliers")
    st.write(f"The most occurring outliers with a count of {max_count} are: {', '.join(most_common_outliers)}")
